...
# python -m pytest -s tests\test_update_grade_class.py -k test_correct_delta_f


# Тестирование загрузки справочника "Классы оценки"

class TestUpdateGradeClass(BaseCase):
...
# Авторизация и получение необходимых cookie и headers
    ...
    def setup(self):
        ...
        self.url = "http://localhost:83/0/rest/SapErpIntegrationService/v1/UpdateGradeClass"
...
# Проверка возможности создания записи без обязательных полей режим "F" (пустой "BKLASS")
    ...
    def test_all_null_ekgrp_value_delta_f(self):
        ...
        obj = json.loads(response.text)
        for result in obj['result']:
            assert result['MESSAGE'] == "Обязательно для заполнения - BKLASS", f"The value of 'MESSAGE' is not correct"
            assert result['TYPE'] == "E", f"The value of 'TYPE' is not correct"

# Проверка возможности создания записи без обязательных полей режим "F" (пустой "BKBEZ")
    ...
    def test_all_null_eknam_value_delta_f(self):
            ...
            obj = json.loads(response.text)
            for result in obj['result']:
                assert result['MESSAGE'] == "Обязательно для заполнения - BKBEZ", f"The value of 'MESSAGE' is not correct"
                assert result['TYPE'] == "E", f"The value of 'TYPE' is not correct"

# Проверка возможности создания записи без обязательных полей режим "D" (пустой "BKLASS")
...
# Проверка возможности создания записи без обязательных полей режим "D" (пустой "BKBEZ")
